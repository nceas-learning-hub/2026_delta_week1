...
An absolutely critical component of version control with Git is what Git _doesn't_ track, not just what it _does_ track. This is useful for many reasons but in particular the .gitignore will be a useful guardrail against tracking (A) large files or (B) files with sensitive information (e.g., passwords, private data).
...
In our earlier exercises, we've started repositories in GitHub with the template ".gitignore" file for R users but when adding Git tracking to an existing folder, we'll need to add the ".gitignore" manually. To do this with Positron, do the following:

1. Enter the **File** menu in the top of the screen
2. Choose **New Text File**
3. Save that file as ".gitignore"

<details>
<summary>Possible Warning on Step 3</summary>
When you save the empty text file as ".gitignore" you may get a warning about creating a file that starts with a period. **Ignore that warning and click whatever button will let you continue** (in the example below it is **Use "."**).

Files and folders that start with a period will be hidden from most file explorer software (e.g., Finder on Mac), but for Git to interpret this file correctly it must be named _exactly_ ".gitignore".

<p align="center">
<img src="images/github_introduction_positron/folder-first-git_03-gitignore-warning.png" alt="Screen capture of Mac warning message about creating a file with a period at the start of the file name" width="40%"/>
</p>

</details>

4. Once you have the .gitignore, go ahead and add the following lines to it:

```
# Ignore Mac "Desktop Services" files
.DS_Store
...
This means that the you won't be able to commit and CSVs, PDFs, or PNGs (as long as those lines are still in your .gitignore anyway!).

## BASEMENT

In this last exercise, we will practice this workflow using your `training_{USERNAME}` folder. Switch to your `training_{USERNAME}` folder using the Positron folder dropdown menu. The folder dropdown menu is in the upper right corner of Positron. Click the dropdown next to your current folder name (`{FIRSTNAME}_test`), and then select the `training_{USERNAME}` project from the list of recent folders you've opened in Positron.
...
There are a few approaches for connecting an existing local folder to Git/GitHub but we'll focus on two: (1) an R-language specific approach using the [`usethis` R package](https://usethis.r-lib.org/) and (2) a language-agnostic method using the command line.
