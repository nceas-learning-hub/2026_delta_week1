...
![](images/synthesis_logic_models/LM3.png){fig-alt="The figure above is modified to include grey arrows pointing right to left between each box, with each box labeled 'if' and each arrow labeled 'then', for example IF resource condition is met THEN activities can result, IF activities THEN participants, etc" width="70%" fig-align="center" .lightbox}
...
![](images/synthesis_logic_models/LM5.png){fig-alt="Multiple boxes, each representing an input, an output, or an outcome, are connected by grey arrows flowing mostly left-to-right but some are bi-directional or indicate loops back to previous steps" width="70%" fig-align="center" .lightbox}
