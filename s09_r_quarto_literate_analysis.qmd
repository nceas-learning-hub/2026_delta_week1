...
!!! Make this standalone - include directions on downloading the data!
Let's build a simple but complete literate analysis in Quarto using real data. We are going to work with the seawater chemistry data. We are going to use the `BGchem2008data.csv` data we downloaded in our previous session.
...
:::{.callout-exercise}
For this analysis, structure your document like this, using Markdown headers below the YAML header:
...
Use `read_csv()` with `here()` to load your data. The `here()` function builds file paths relative to your project root, which keeps things reproducible regardless of where your `.qmd` file lives within the project (including if you move it to a new location within the project!).
...
::::{.callout-hint}
Check the code chunk options above!  Alternately, check the documentation for `read_csv()` (`?read_csv`) to see an option for suppressing this behavior!
::::
:::
...
**Rule of thumb:** If your document renders successfully, it's reproducible. Get into the habit of rendering regularly during development.
...
PDF rendering requires a LaTeX installation. If you don't have one, the easiest fix is `tinytex`:
