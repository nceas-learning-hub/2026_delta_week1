...
-   Apply Markdown syntax to format and structure prose (non-code) text in a Quarto document
...
***Quarto*** (and its predecessor RMarkdown) is an open-source scientific publishing system excellent way to generate literate analysis and a reproducible workflow. Quarto files interleave sections of formatted prose and sections of functional code ("code chunks") that can be rendered to create a complete record of an analysis, including the code, the results, and the interpretation of the results.

:::{.callout-tip}
### Coding languages in Quarto

We will focus on using Quarto with R code, but it is designed to work seamlessly with other coding languages as well, including Python, Julia, and Observable JavaScript. You can even use multiple languages in the same document to integrate different tools into a single workflow.
:::
...
Reproducible analysis allow you to automatize how the figures and the statistics in your analysis are generated from raw data to final results.  This process also helps your collaborators, your readers, and your future self to follow your code trail the leads to the original data, increasing the transparency of your science.
