...
## Splitting or Combine Columns Using `separate_wider_delim()` and `unite()`

The `separate_wider_delim()` function allow us to easily split a single column into numerous. Its complement, the `unite()` function, allows us to combine multiple columns into a single one.
...
sites_df %>%
    separate_wider_delim(cols = site, names = c("island", "site_number"), delim = "-")
```
...
cities_clean <- cities_df %>%
    separate_wider_delim(city, c("city", "state_code"), " ")

```
:::

The `unite()` function does just the reverse of `separate_wider_delim()`. If we have a `data.frame` that contains columns for year, month, and day, we might want to unite these into a single date column.
